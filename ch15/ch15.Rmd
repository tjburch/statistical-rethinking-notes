...
Imputed values are shown in blue with 89% compaitbility intervals in the line segments - of course there's a lot of uncertainty. The second plot shows the relationship between the predictors, no upward slope since the regression assumed no relationship.
...
Discrete variables are more challenging - no flat surface for HMC, calibration is hard. Instead of sampling, do a _weighted average_ to remove discrete parameters - after sampling other parameters, use their damples to compute posterior of any discrete parameter removed.
...
When $C_i$ isn't known, compute likelihood of $N_i$ for each value and anverage. Can do in stan or ulam:
...
The new lines are the imputed values. The first defines a vector to hold probabilities (exponential stuff is beause we want to work on log scale). Next two lines are likelihoods as before - cat present `lpC1` and cat absent `lpC0`.
...
This example concerned missing data, but when measured with error, apporach is similar - make probability of a cat $k_i$, and replace the parameter $k$ with it.
